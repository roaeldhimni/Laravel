...
<header>
  ...
  <div class="container">
    <br><br>
    <h1 class="text-center my-5">Liste des salles:</h1>
    <table class="table table-striped table-bordered" id="salle-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $salle->libelle }}</td>
          <td>
            <form method="GET" action="{{ route('salles.edit', $salle->id) }}">
              @csrf
              <!-- <button type="submit" class="btn btn-info btn-sm">Modifier</button> -->
              <a href="{{ route('salles.edit', $salle->id) }}" class="btn btn-info btn-sm">Modifier</a>
            </form>
          </td>
          <td>
    ...
    <div class="row mt-5">
      <div class="col-md-6">
        ...
        <a href="{{ route('salles.create') }}" class="btn btn-success" id="btnAjouter">Ajouter salle</a>
      </div>
  ...
  <script>
    ...
    $('#btnSupprimerTout').on('click', function() {
      // Ajouter ici le code pour supprimer toutes les salles
      alert('Supprimer toutes les salles');
    });
  </script>
</body>
</html>